...
output "dsf_hub_ssh_key" {
  sensitive = true
  value     = module.key_pair_hub.key_pair.key_pair_private_pem
}
...
output "ssh_command_hub" {
  value = "ssh -i ${module.key_pair_hub.key_pair.key_pair_private_pem.filename} ec2-user@${module.hub.public_address}"
}

output "ssh_command_gw" {
  value = try("ssh -o UserKnownHostsFile=/dev/null -o ProxyCommand='ssh -o UserKnownHostsFile=/dev/null -i ${module.key_pair_hub.key_pair.key_pair_private_pem} -W %h:%p ec2-user@${module.hub.public_address}' -i ${module.key_pair_hub.key_pair.key_pair_private_pem} ec2-user@${module.agentless_gw_group[0].private_address}", null)
}
