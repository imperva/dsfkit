...
module "globals" {
  source = "../../../deploy/modules/core/globals"
}

module "key_pair" {
  source                   = "../../../deploy/modules/core/key_pair"
...
resource "aws_key_pair" "deployer" {
  ...
  public_key = module.key_pair.key_pair.public_key_openssh
  provider   = aws.second
}
...
module "vpc2" {
  ...
  azs             = slice(data.aws_availability_zones.available2.names, 0, 2)
  private_subnets = var.private_subnets
  public_subnets  = var.public_subnets
...
module "hub" {
  ...
  ingress_communication = {
    ...
    full_access_cidr_list = concat(local.workstation_cidr, ["3.141.124.190/32"]) #["${module.hub_secondary.public_dns}/32"])
    use_public_ip = true
  }
  depends_on = [
    module.vpc
...
module "hub_secondary" {
  ...
  ingress_communication = {
    additional_web_console_access_cidr_list = var.web_console_cidr
    full_access_cidr_list = concat(local.workstation_cidr, ["44.206.106.191/32"]) #["${module.hub.public_dns}/32"])
    use_public_ip = true
  }
  depends_on = [
    module.vpc2
...
#   count                             = var.gw_count
#   source = "../../../deploy/modules/agentless-gw"
...
#   source = "../../../deploy/modules/federation"
#   gws_info  = {
...
module "hadr" {
  source = "../../../deploy/modules/hadr"
  dsf_hub_primary_public_ip    = module.hub.public_dns
  dsf_hub_primary_private_ip   = module.hub.public_dns
  dsf_hub_secondary_public_ip  = module.hub_secondary.public_dns
  dsf_hub_secondary_private_ip = module.hub_secondary.public_dns
...
# module "rds_mysql" {
#   count                        = 1
#   source = "../../../deploy/modules/rds-mysql-db"
...
#   source = "../../../deploy/modules/db-onboarder"
...
# module "statistics" {
#   source = "../../../deploy/modules/statistics"
# }
