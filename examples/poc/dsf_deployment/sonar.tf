locals {
  ...
  hub_main_address        = var.enable_sonar ? (module.hub_main[0].public_ip != null ? module.hub_main[0].public_ip : module.hub_main[0].private_ip) : null
  hub_dr_address          = var.enable_sonar && var.hub_hadr ? (module.hub_dr[0].public_ip != null ? module.hub_dr[0].public_ip : module.hub_dr[0].private_ip) : null
  hub_cidr                = try(format("%s/32", local.hub_main_address), null)
  hub_dr_cidr             = try(format("%s/32", local.hub_dr_address), null)
  hub_cidr_list           = var.enable_sonar ? concat([data.aws_subnet.hub.cidr_block, data.aws_subnet.hub_dr.cidr_block], var.public_ip ? [local.hub_cidr, local.hub_dr_cidr] : []) : []
