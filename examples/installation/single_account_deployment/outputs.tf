  ...
  value = {
    for idx, val in module.agentless_gw_group : "gw-${idx}" => {
      primary = {
        ...
        ssh_command  = try("ssh -o ProxyCommand='ssh -o UserKnownHostsFile=/dev/null -i ${module.key_pair_hub.key_pair_private_pem.filename} -W %h:%p ${module.hub_primary.ssh_user}@${module.hub_primary.private_ip}' -i ${module.key_pair_gw.key_pair_private_pem.filename} ${module.agentless_gw_group[idx].ssh_user}@${module.agentless_gw_group[idx].private_ip}", null)
      }
    }
  }
}
...
output "dsf_hubs" {
  value = {
    primary = {
      ...
      ssh_command  = try("ssh -i ${module.key_pair_hub.key_pair_private_pem.filename} ${module.hub_primary.ssh_user}@${module.hub_primary.private_ip}", null)
    }
    secondary = {
