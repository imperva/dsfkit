  ...
  value = {
    for idx, val in module.agentless_gw1_group : "gw1-${idx}" => {
      primary = {
        ...
        ssh_command  = try("ssh -o ProxyCommand='ssh -o UserKnownHostsFile=/dev/null -i ${var.hub_private_key_pem_file_path} -W %h:%p ${var.hub_ssh_user}@${var.hub_private_ip}' -i ${nonsensitive(module.key_pair_gw1.key_pair_private_pem.filename)} ${module.agentless_gw1_group[idx].ssh_user}@${module.agentless_gw1_group[idx].private_ip}", null)
      }
    }
  }
}
  ...
  value = {
    for idx, val in module.agentless_gw2_group : "gw2-${idx}" => {
      primary = {
        ...
        ssh_command  = try("ssh -o ProxyCommand='ssh -o UserKnownHostsFile=/dev/null -i ${var.hub_private_key_pem_file_path} -W %h:%p ${var.hub_ssh_user}@${var.hub_private_ip}' -i ${nonsensitive(module.key_pair_gw2.key_pair_private_pem.filename)} ${module.agentless_gw2_group[idx].ssh_user}@${module.agentless_gw2_group[idx].private_ip}", null)
      }
    }
  }
}
