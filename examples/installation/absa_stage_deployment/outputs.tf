output "dsf_agentless_gw_group" {
  ...
  for idx, val in module.agentless_gw_group : "gw-${idx}" =>
  {
    ...
    ssh_command  = try("ssh -o ProxyCommand='ssh -o UserKnownHostsFile=/dev/null -i ${var.private_key_pem_file_path} -W %h:%p ${module.hub.ssh_user}@${module.hub.private_ip}' -i ${var.private_key_pem_file_path} ${val.ssh_user}@${val.private_ip}", null)
  }
  }
}
...
output "dsf_hubs" {
  value = {
    primary_hub = {
      ...
      ssh_command  = try("ssh -i ${var.private_key_pem_file_path} ${module.hub.ssh_user}@${module.hub.private_ip}", null)
    }
  }
}
