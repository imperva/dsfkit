locals {
  bastion_host        = try(var.wh_proxy_info.ip_address, null)
  bastion_private_key = try(file(var.wh_proxy_info.private_ssh_key_path), "")
  bastion_user        = try(var.wh_proxy_info.ssh_user, null)
