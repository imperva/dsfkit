...
class UpgradeStatusService:
    ...
    def should_collect_node_info(self, dsf_node_id):
        status = self.get_upgrade_status(dsf_node_id)
        return status not in (
            UpgradeStatus.SUCCEEDED,
            UpgradeStatus.SUCCEEDED_WITH_WARNINGS,
    ...
    def _is_overall_upgrade_status_running(self, statuses):
        ...
        running_count = statuses.count(UpgradeStatus.RUNNING_TEST_CONNECTION) \
            ...
            + statuses.count(UpgradeStatus.RUNNING_COLLECT_NODE_INFO) \
            + statuses.count(UpgradeStatus.COLLECT_NODE_INFO_SUCCEEDED) \
...
class UpgradeStatus(Enum):
    ...
    RUNNING_COLLECT_NODE_INFO = "Running collect node info"
    COLLECT_NODE_INFO_SUCCEEDED = "Collect node info succeeded"
    COLLECT_NODE_INFO_FAILED = "Collect node info failed"
