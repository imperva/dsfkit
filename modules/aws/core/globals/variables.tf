...
variable "sonar_version" {
  ...
  validation {
    condition     = contains(local.sonar_supported_versions, var.sonar_version)
    error_message = "The sonar_version value must be 4.9 or higher"
  }
}
...
variable "dra_version" {
  ...
  validation {
    condition     = contains(local.dra_supported_versions, var.dra_version)
